...
# ceilings from dimuonX_config.json func_forms (for rail detection)
CEIL = {"bern": 7, "polyExp": 4, "exp": 5, "expPoly": 4}
FAMILY_ORDER = ["bern", "polyExp", "exp", "expPoly"]  # order they appear in the log


def sigma_of(mass):
    with open(f"signal_fits/2B_loosemass/case_interpolation_M{mass}.0.json") as f:
...
def run_one(job):
    ...
    cmd = [
        sys.executable, os.path.join(FRAMEWORK_DIR, "doFit.py"), "-c", "dimuonX_config.json", "-M", str(mass),
        "--m-min", str(lo), "--m-max", str(hi), "--bin-size", str(bs),
        "--sig_norm", str(n_inj), "-i", f"mock_data/mock_M{mass}.h5",
        "-s", f"signal_fits/2B_loosemass/sig_fit_{mass}.json", "-o", outdir,
    ]
