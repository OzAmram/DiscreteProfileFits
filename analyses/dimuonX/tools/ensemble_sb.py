...
own in-window background), fit through `run_fit_adaptive.py` -- the SAME wrapper
the data scan uses -- and record:
...
Because the adaptive wrapper chooses the window itself, N_inj_in_window cannot be
known up front: it is counted AFTER the fit, over the winning window read back
from the results JSON (m_min/m_max). The injected events are saved per job so the
count is exact rather than re-simulated.
"""
import json
import os
import subprocess
...
# Adaptive-search settings -- mirror run_data_scan.py's defaults.
...
def sig_params(mass):
    # Use the INTERPOLATED template -- that is what run_fit_adaptive.py fits with
    # and what it sizes its window from, so the mock must be built against it.
    with open(os.path.join(SIGDIR, f"case_interpolation_M{float(mass)}.json")) as f:
        p = json.load(f)
...
def build_mock(k, mass):
    """Return (mock_path, inj_path).

    The signal is injected over the WIDEST window the adaptive wrapper can use
    (+-N_SIGMA), which is a superset of whatever window it settles on, so the
    mock does not depend on the wrapper's choice. The injected events are saved
    so the in-window truth can be counted exactly once that choice is known.
    """
    rng = np.random.default_rng(5000 + k * 100 + mass)
    bkg = subsample(k)
    sig = load_sig(mass)
    mean, sigma = sig_params(mass)
    m_min, m_max, _ = window(mass, sigma)
    ...
    ip = os.path.join(d, "injected.h5")
    with h5py.File(ip, "w") as f:
        f.create_dataset("masses", data=inj.astype(np.float32), compression="gzip",
...
def run_one(job):
    k, mass = job
    mp, ip = build_mock(k, mass)
    outdir = os.path.join(OUTROOT, f"seed{k}", f"M{mass}")
    os.makedirs(outdir, exist_ok=True)
    # Same driver + flags as run_data_scan.py. sig_norm is left at the config
    # value (as in data): obs_excess_events = r*sig_norm is in events either way,
    # so the pull is unaffected by that choice.
    cmd = [
    ...
    # Count the injected truth over the window the wrapper actually chose.
    rj = os.path.join(outdir, "best", f"fit_results_{float(mass)}.json")
    n_inj = None
    if os.path.exists(rj):
        r = json.load(open(rj))
        m_min, m_max = r.get("m_min"), r.get("m_max")
        if m_min is not None and m_max is not None:
            with h5py.File(ip) as f:
                inj = f["masses"][:]
...
def main():
    ...
    for M in MASSES:
        ...
        for k in range(K):
            ...
            rj = os.path.join(base, "best", f"fit_results_{float(M)}.json")
