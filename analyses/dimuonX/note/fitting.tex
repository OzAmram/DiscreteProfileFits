...
However for a narrow width signal the width of the observed resonance will be dictated only by the experimental resolution on the di-muon mass rather than the specific signal properties.
We therefore use the signal shape obtained from a fit to a chosen narrow width signal as a generic 'narrow width' shape which can be employed for the search.
...
Since we do not know the exact mass shapes after the application of the anomaly score, we first do tests on the background 
...
At 75\GeV a coherent negative offset is observed, with a median pull between $-0.63$ and $-0.27$; all
twelve configurations at this mass are negative and one of them, toys generated from a polynomial times an
exponential with a five standard deviation signal injected, exceeds the acceptance criterion.
This is the highest mass considered, where the nominal $\pm 7\sigma$ window extends beyond the upper edge
of the available mass range and is truncated at 80\GeV, leaving sideband on only one side of the
resonance and reducing the constraint on the background shape.
The behaviour of the fit at the upper edge of the search range is under study.
...
The tests above use background simulation prior to the anomaly score selection, since the mass shape after the cut was not characterized at the time they were designed.
This is now repeated using simulated signal-region background processed through the actual CATHODE pipeline -- the same generative model, classifier, and anomaly selection used on data -- providing the closest available proxy of the real analysis chain.
The bump-hunt fit is performed at 97 mass hypotheses between 13.8 and 75.0\GeV, distributed over 19 mass windows.
A narrow band, 47.5--52.5\GeV, is excluded from the scan: the current background simulation sample is deficient in this region, a known issue with the simulation unrelated to the analysis method, and every mass hypothesis whose nominal $\pm7\sigma$ fit window would overlap it is dropped.
