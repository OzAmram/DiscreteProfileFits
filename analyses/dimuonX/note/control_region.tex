...
By conservation of charge, any resonant BSM process must yield an opposite-sign pair, so the same-sign sample is free of such contributions; SM backgrounds arise from non-prompt and misidentified muons, with no reason to expect peaking structures in the invariant mass distribution.
This makes the same-sign region an ideal testbed for CATHODE: a smooth, signal-free background allows verification that the method does not generate spurious excesses in the absence of true new physics, and any sculpting artifacts introduced by the anomaly detection pipeline can be identified before unbinding.
...
The control region dataset consists of the 2024 CMS scouting dataset with Eras E--I, corresponding to an integrated luminosity of $96.94~\fbinv$ after the certified good-run selection.
...
\newpage
%------------------------------------------------------------------
\subsubsection{Control Region CATHODE Configuration}
\label{sec:cr_cathode_config}
%------------------------------------------------------------------

The generative model and classifier configurations used in the control region validation are identical to those described in Sections~\ref{sec:generative_model} and~\ref{sec:xgboost}, respectively.
The key parameters are summarized here for convenience.

A Conditional Flow Matching (CFM) model is trained on sideband events in each mass window.
The vector field network is a multilayer perceptron with 8 hidden layers of 256 neurons each, using ELU activation functions.
The mass conditioning variable $m_{\mu\mu}$ is concatenated with the feature vector and sinusoidal time embeddings at each forward pass.
Training uses the Adam optimizer with a cosine annealing learning rate schedule, starting at $1\times10^{-3}$ and decaying to $1\times10^{-7}$, for 1000 epochs.
Synthetic background events are generated by integrating the learned vector field from the standard normal prior using 500 integration steps.
The number of synthetic events generated per fold is 3 the number of real SR events, following the prescription of Ref.~\cite{CATHODE}.

An XGBoost gradient-boosted decision tree ensemble is trained to distinguish real SR data (label~$=1$) from synthetic background (label~$=0$).
The classifier uses 1000 boosting rounds with a maximum tree depth of 3, a learning rate of 0.05, early stopping, and standard settings otherwise.
A 5-fold cross-validation scheme is employed to ensure no event is scored by a classifier trained on it.

After the 1\% most anomalous events are selected by the classifier, the signal-plus-background fit described in Section~\ref{sec:stat_procedure} is performed on the $m_{\mu\mu}$ distribution.
The background is described by the full discrete-profiling envelope of four functional-form families (Bernstein polynomial, sum of exponentials, polynomial times exponential, and exponential of a polynomial), with the order of each family fixed by the F-test and the fit window and binning set adaptively, exactly as in the signal region.
The fit returns the best-fit signal yield, its uncertainty, and the $\chi^2/\mathrm{ndof}$ and corresponding $p$-value of the post-fit model.
...
\begin{figure}[!h]
    ...
    \caption{Background-only goodness of fit over the 119 mass hypotheses of the same-sign control region scan.
    Top: the background-only goodness-of-fit $p$-value as a function of the di-muon mass hypothesis, with the $p = 0.05$ threshold indicated.
...
\begin{figure}[!h]
    ...
    The significances scatter symmetrically about zero and lie between $-2.2\sigma$ and $+2.6\sigma$ over the full range, with the largest excursion, $+2.6\sigma$, at 67.0\GeV.}
