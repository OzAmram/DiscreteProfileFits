"""
Preprocess the UAF_training signal models for signal-shape fitting.

Each postprocessed signal file stores the dimuon mass in 'Background_dimuon_mass'
and a large feature set; fit_signalshapes.py wants a plain h5 with key 'masses'.
This extracts the mass column per file into signal_data_uaf/<group>/mass_<M>.h5,
grouping files by process+heavy-parent (the resonance mass is the last mass token
in the filename). Writes signal_uaf_manifest.json: {group: [[mass, h5path], ...]}.
"""
import glob
import json
import os
...
SRC = "/eos/uscms/store/user/tvami/CATHODE/UAF_training"
OUT = "signal_data_uaf"
...
for g in manifest:
    manifest[g].sort()
json.dump(manifest, open("signal_uaf_manifest.json", "w"), indent=2)
...
print("wrote signal_uaf_manifest.json")
