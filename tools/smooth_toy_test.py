...
def win(mass):
    s = json.load(open("signal_fits/2B/sig_fit_%d.json" % mass))["sigma"]
...
def run_one(job):
    ...
    cmd = [sys.executable, "doFit.py", "-c", "bkg_mc_config.json", "-M", str(mass),
           "--m-min", str(lo), "--m-max", str(hi), "--bin-size", str(bs),
           "--sig_norm", "100", "-i", tp,
           "-s", "signal_fits/2B/sig_fit_%d.json" % mass, "-o", outdir]
